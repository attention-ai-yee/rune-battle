  ...
  return (
    <div
      ...
      className={`
        ...
        ${isTargetable ? 'cursor-pointer hover:border-rune-gold hover:bg-gray-800/80' : ''}
    ...
    >
      ...
      {/* Intent indicator */}
      <div className="absolute -top-8 left-1/2 -translate-x-1/2 flex items-center gap-1 px-2 py-1 rounded bg-rune-dark/90 border border-gray-700/50 animate-enemy-intent">
        <span className="text-xs">{intentInfo.icon}</span>
        <span className={`text-xs font-bold ${intentInfo.color}`}>
          {enemy.intent.type === 'buff' ? `+${enemy.intent.value}` : enemy.intent.value}
        </span>
        <span className="text-[10px] text-gray-400">{enemy.intent.description}</span>
      </div>

      {/* Enemy emoji/avatar */}
      <div className={`text-5xl mb-2 ${enemy.isFrozen ? 'opacity-60' : 'animate-float'}`} style={{ animationDelay: `${index * 0.5}s` }}>
        {enemy.emoji}
      </div>

      {/* Enemy name */}
      <h3 className="text-sm font-bold text-gray-200 mb-1">{enemy.name}</h3>

      {/* Status effects */}
      {enemy.statusEffects.length > 0 && (
        <div className="flex items-center gap-1 mb-1">
          {poisonStacks > 0 && (
            <span className="text-xs bg-green-900/40 px-1.5 py-0.5 rounded">🧪 {poisonStacks}</span>
          )}
          {burnStacks > 0 && (
            <span className="text-xs bg-orange-900/40 px-1.5 py-0.5 rounded">🔥 {burnStacks}</span>
          )}
          {freezeStacks > 0 && (
            <span className="text-xs bg-cyan-900/40 px-1.5 py-0.5 rounded">❄️ {freezeStacks}</span>
          )}
        </div>
      )}
      ...
      {enemy.strength > 0 && (
        <div className="flex items-center gap-0.5 mb-1">
          ...
          <span className="text-xs text-rune-purple font-bold">+{enemy.strength}</span>
        </div>
      )}

      {/* HP Bar */}
      <div className="w-full max-w-[120px] mb-1">
      ...
      {enemy.armor > 0 && (
        <div className="flex items-center gap-1 mt-1">
          ...
          <span className="text-xs text-rune-blue font-bold">{enemy.armor}</span>
        </div>
      )}
