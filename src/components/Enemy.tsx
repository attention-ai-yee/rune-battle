  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-7 sm:-top-8 left-1/2 -translate-x-1/2 flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded bg-rune-dark/90 border border-gray-700/50 animate-enemy-intent whitespace-nowrap">
        <span className="text-[9px] sm:text-xs">{intentInfo.icon}</span>
        <span className={`text-[9px] sm:text-xs font-bold ${intentInfo.color}`}>
          {enemy.intent.type === 'buff' ? `+${enemy.intent.value}` : enemy.intent.value}
        </span>
        <span className="text-[7px] sm:text-[10px] text-gray-400 hidden sm:inline">{enemy.intent.description}</span>
      </div>
      ...
      {/* Enemy name */}
      <h3 className="text-[10px] sm:text-sm font-bold text-gray-200 mb-0.5 sm:mb-1">{enemy.name}</h3>

      {/* Status effects */}
      {enemy.statusEffects.length > 0 && (
        <div className="flex items-center gap-0.5 sm:gap-1 mb-0.5 sm:mb-1">
          {poisonStacks > 0 && (
            <span className="text-[8px] sm:text-xs bg-green-900/40 px-1 sm:px-1.5 py-0 sm:py-0.5 rounded">🧪 {poisonStacks}</span>
          )}
          {burnStacks > 0 && (
            <span className="text-[8px] sm:text-xs bg-orange-900/40 px-1 sm:px-1.5 py-0 sm:py-0.5 rounded">🔥 {burnStacks}</span>
          )}
          {freezeStacks > 0 && (
            <span className="text-[8px] sm:text-xs bg-cyan-900/40 px-1 sm:px-1.5 py-0 sm:py-0.5 rounded">❄️ {freezeStacks}</span>
          )}
        </div>
      )}
      ...
      {enemy.strength > 0 && (
        <div className="flex items-center gap-0.5 mb-0.5 sm:mb-1">
          ...
          <span className="text-[8px] sm:text-xs text-rune-purple font-bold">+{enemy.strength}</span>
        </div>
      )}

      {/* HP Bar */}
      <div className="w-full max-w-[80px] sm:max-w-[120px] mb-0.5 sm:mb-1">
      ...
      {enemy.armor > 0 && (
        <div className="flex items-center gap-0.5 sm:gap-1 mt-0.5 sm:mt-1">
          <span className="text-[8px] sm:text-xs">🛡️</span>
          <span className="text-[8px] sm:text-xs text-rune-blue font-bold">{enemy.armor}</span>
        </div>
      )}
